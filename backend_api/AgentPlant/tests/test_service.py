...
def test_create_artifact_service(tmp_path):
    from backend_api.AgentPlant.schemas import ArtifactCreateRequest, PlantPayload, PreLaunchConfig
    ...
    request = ArtifactCreateRequest(plant=plant, pre_launch=pre_launch)

    validation = validate_plant_and_pre_launch(
        {"system_name": plant.system_name, "python_code": plant.python_code},
        {
            "total_simulation_time": 5.0,
            "solver_sample_time": 0.01,
            "initial_state": [],
    ...
    created = create_artifact(request, store=store)
    assert created.artifact_id
    assert created.system_name == "simple_integrator"

    summaries = list_artifacts(store=store)
    assert any(s.artifact_id == created.artifact_id for s in summaries)

    detail = get_artifact(created.artifact_id, store=store)
    assert detail.plant["python_code"].startswith("def dynamics")

    plugin = get_artifact_plugin(created.artifact_id, store=store)
    assert "SimpleIntegratorDynamics" in plugin.source or "Dynamics" in plugin.source

    spec = get_adaptive_spec(created.artifact_id, store=store)
    assert spec["system_name"] == "simple_integrator"

    with pytest.raises(ArtifactValidationError):
        create_artifact(
            ArtifactCreateRequest(
                plant=PlantPayload(system_name="", python_code="pass"),
